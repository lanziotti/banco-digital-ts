...
export class TransferController {
    async create(req: Request, res: Response) {
        ...
        if (!destinationAccount) {
            throw new NotFoundError("Essa conta de destino não existe. Por favor, verifique se o número da conta de destino está correta.");
        }

        if (valor <= 0) {
            throw new NotFoundError("Não é possível fazer uma TRANSFERÊNCIA com valores zerados ou negativos.");
        }
